...
public class Mario : MonoBehaviour
{
	...
	void Update()
	{
	    if (Input.GetKey(KeyCode.LeftArrow)) // 왼쪽키가 눌린 경우
        {
            ...
            if (GetComponent<Rigidbody2D>().velocity.x > -10f) GetComponent<Rigidbody2D>().AddRelativeForce(Vector2.right * -15f); // X축 가속값이 -10 이상인 경우 오른쪽으로 가속시킨다.
        }
        else if (Input.GetKey(KeyCode.RightArrow))
        {
            transform.localScale = new Vector3(1f, 1f, 1f);
			if (GetComponent<Rigidbody2D>().velocity.x < 10f) GetComponent<Rigidbody2D>().AddRelativeForce(Vector2.right * 15f); // X축 가속값이 10 이하인 경우 왼쪽으로 가속시킨다.
        }
        ...
        if (Input.GetKeyDown(KeyCode.Space)) // 점프키를 눌렀을때
        {
            ...
            {
                GetComponent<AudioSource>().Play(); // 점프 사운드 연주
                GetComponent<Rigidbody2D>().AddForce(Vector2.up * 700f); // 위쪽으로 700f의 파워로 점프
                anim.SetTrigger("jump"); // 점프 애니메이션 실행
            }
        }

        if (GetComponent<Rigidbody2D>().velocity.magnitude >= 0.1f) // 가속력이 0.1f 보다 크면 키보드를 누르지 않고 있어도 움직이고 있다고 판단해 달리기 애니메이션을 한다.
        {
